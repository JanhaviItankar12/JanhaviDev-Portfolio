...
const Dashboard = () => {
  ...
  const handleProjectImageUpload = (e, isEditing = false) => {
  ...
  // Handle logo upload for experience
  ...
  const removeProjectImage = (isEditing = false) => {
    if (isEditing) {
      setEditingItem({
        ...editingItem,
        imageFile: null,
  ...
  const removeCompanyLogo = (isEditing = false) => {
    if (isEditing) {
      setEditingItem({
        ...editingItem,
  ...
  const addProject = async () => {
    try {
      ...
      alert(error?.data?.message || 'Failed to create project. Please try again.');
    }
  };
  ...
  const updateProjectHandler = async () => {
    try {
      ...
      formData.append("technologies", editingItem.technologies);
      formData.append("github", editingItem.github);
      formData.append("live", editingItem.live);
      ...
      await updateProject({ id: editingItem._id, formData }).unwrap();
      await refetchProjects();
  ...
  const addExperience = async () => {
    try {
      ...
      alert(error?.data?.message || 'Failed to create experience. Please try again.');
    }
  };

  // Handle edit experience initialization
  const handleEditExperience = (exp) => {
    setEditingItem({ 
      ...
      companyLogoFile: null,
      companyLogoPreview: exp.companyLogo
    });
    setShowEditModal(true);
  };
  ...
  const updateExperienceHandler = async () => {
    try {
      const experienceData = {
        ...
        endDate: editingItem.currentlyWorking ? undefined : (editingItem.endDate ? new Date(editingItem.endDate) : undefined),
        currentlyWorking: editingItem.currentlyWorking,
        companyLogo: editingItem.companyLogoFile || editingItem.companyLogo,
        location: editingItem.location
      };

      await updateExperience({ id: editingItem._id, experienceData }).unwrap();
      await refetchExperiences();
      ...
      alert(error?.data?.message || 'Failed to update experience');
    }
  };
  ...
  const deleteExperienceHandler = async (id) => {
    if (window.confirm('Are you sure you want to delete this experience?')) {
      try {
        ...
        alert(error?.data?.message || 'Failed to delete experience');
      }
    }
  };
      ...
      <AnimatePresence>
        {showEditModal && editingItem && (
          ...
          >
            ...
            >
              <h2 className="text-xl md:text-2xl font-bold mb-4 text-white">
                Edit {activeTab === 'projects' ? 'Project' : 'Experience'}
              </h2>
              ...
              {activeTab === 'projects' ? (
                <div className="space-y-4">
                    ...
                    <div className="relative">
                      {editingItem?.imagePreview ? (
                        <div className="relative">
                          ...
                          />
                          <button
                            onClick={() => removeProjectImage(true)}
                            className="absolute top-2 right-2 p-1 rounded-full bg-red-500 text-white hover:bg-red-600"
                          >
                            <FiClose size={16} />
                          </button>
                  ...
                  <input
                    type="text"
                    placeholder="Live Demo URL"
                    value={editingItem?.live || ''}
                    onChange={(e) => setEditingItem({ ...editingItem, live: e.target.value })}
                    className="w-full px-4 py-2 rounded-lg text-sm md:text-base"
                    style={{ backgroundColor: 'var(--color-dark-300)' }}
                  />
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <label htmlFor="edit-featured" className="text-sm text-gray-300">Featured Project</label>
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm text-gray-400">Company Logo</label>
                    <div className="relative">
                      {editingItem?.companyLogoPreview ? (
                        <div className="relative w-20 h-20 md:w-24 md:h-24">
                          <img 
                            src={editingItem.companyLogoPreview} 
                            alt="Company Logo" 
                            className="w-full h-full object-cover rounded-lg"
                          />
                          <button
                            onClick={() => removeCompanyLogo(true)}
                            className="absolute -top-2 -right-2 p-1 rounded-full bg-red-500 text-white hover:bg-red-600"
                          >
                            <FiClose size={14} />
                          </button>
                        ...
                        <div 
                          className="w-20 h-20 md:w-24 md:h-24 rounded-lg flex flex-col items-center justify-center cursor-pointer border-2 border-dashed transition-colors hover:border-[var(--color-accent-primary)]"
                          style={{ borderColor: 'var(--color-dark-400)' }}
                          onClick={() => document.getElementById('edit-company-logo').click()}
                        >
                          <FiImage className="text-2xl md:text-3xl mb-1" style={{ color: 'var(--color-accent-primary)' }} />
                          <p className="text-xs text-gray-400">Upload Logo</p>
                        </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <label htmlFor="edit-currentlyWorking" className="text-sm text-gray-300">Currently Working</label>
                  </div>
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 mt-6">
                <button
                  onClick={activeTab === 'projects' ? updateProjectHandler : updateExperienceHandler}
                  disabled={isUploading || isUpdatingProject || isUpdatingExperience}
                  className="w-full sm:flex-1 px-4 py-2 rounded-lg text-white disabled:opacity-50 disabled:cursor-not-allowed"
                  style={{ backgroundColor: 'var(--color-accent-secondary)' }}
                >
                  {isUploading ? 'Uploading...' : isUpdatingProject || isUpdatingExperience ? 'Saving...' : 'Save Changes'}
                </button>
                <button
                  onClick={() => {
                    setShowEditModal(false);
